...
$server->register(
        'update_login_details', array('fieldname' => 'tns:common_array'), array('return' => 'xsd:string'), $NAMESPACE);

$server->register(
        'send_mail_for_password', array('email' => 'xsd:string'), array('return' => 'xsd:string'), $NAMESPACE);
...
function get_combo_values($input_array) {
    ...
    $result751 = $adb->pquery("select vtiger_cf_751.cf_751 from vtiger_cf_751 order by sortorderid", array());
    for ($i = 0; $i < $adb->num_rows($result751); $i++) {
        $output['cf_751']['cf_751'][$i] = $adb->query_result($result751, $i, "cf_751");
    }


    $result753 = $adb->pquery("select vtiger_cf_753.cf_753 from vtiger_cf_753 order by sortorderid", array());
    for ($i = 0; $i < $adb->num_rows($result753); $i++) {
        $output['cf_753']['cf_753'][$i] = $adb->query_result($result753, $i, "cf_753");
    }


    $result755 = $adb->pquery("select vtiger_cf_755.cf_755 from vtiger_cf_755 order by sortorderid", array());
    for ($i = 0; $i < $adb->num_rows($result755); $i++) {
        $output['cf_755']['cf_755'][$i] = $adb->query_result($result755, $i, "cf_755");
    }
...
function create_ticket($input_array) {
    ...
    $cf_751 = $input_array['cf_751'];
    $cf_753 = $input_array['cf_753'];
    $cf_755 = $input_array['cf_755'];
    ...
    $ticket->column_fields[cf_751] = $cf_751;
    $ticket->column_fields[cf_753] = $cf_753;
    $ticket->column_fields[cf_755] = $cf_755;
